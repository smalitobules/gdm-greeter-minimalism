...
verify_overlay_resources() {
    ...
    G_RESOURCE_OVERLAYS="${env_value}" gjs -c "const Gio = imports.gi.Gio; const ByteArray = imports.byteArray; const data = Gio.resources_lookup_data('/org/gnome/shell/ui/screenShield.js', 0); const text = ByteArray.toString(data.toArray()); if (!text.includes('org.gnome.DisplayManager.LocalDisplayFactory') || !text.includes('CreateTransientDisplay') || !text.includes('this._greeterSwitchTimeoutId = GLib.timeout_add(') || !text.includes(\"St.Clipboard.get_default().set_text(St.ClipboardType.CLIPBOARD, '')\") || !text.includes(\"St.Clipboard.get_default().set_text(St.ClipboardType.PRIMARY, '')\") || !text.includes('Gio.DBus.system.call(') || text.includes('Util.spawn([') || text.includes('Screen lock is locked down') || text.includes('AccountsService.UserManager.get_default();')) throw new Error('screenShield lookup fehlgeschlagen');"
